...
<!-- 主体部分 -->
      ...
      <el-table-column prop="updateTime" label="最后修改时间"></el-table-column>
       <el-table-column prop="isDeleted" label="是否能被删除"></el-table-column>
          ...
          <el-button 
                      type="success" class="el-icon-menu"
            ...
            <el-button type="primary" @click="handleEdit(scope.row)" >编辑 <i class="el-icon-edit"></i
          ></el-button>
    ...
    <!-- 角色信息框 -->
		<el-dialog title="角色信息栏" :visible.sync="dialogFormVisible" width="30%">
...
-->
				<span class="custom-tree-node" slot-scope="{ node, data }">
					<span>
						<i :class="data.icon" style="font-size:15px"></i> 
          <span style="padding-left:10px; font-size:10px">  {{ data.name }}</span>
					</span>
				</span>
			</el-tree>
			<div slot="footer" class="dialog-footer">
  ...
  methods: {
    ...
    handleSelectMenu(row) {
      ...
      this.request.post("/role/selectMenuByRole/" + this.roleId).then(res => {
        ...
        this.request.post("menu/findAllMenu").then(res => {
          const allIdList = res.data; // 将res.data赋值给allIdList
        ...
        this.request.post("menu/findAl").then(res => {
          this.menuData = res.data;
        });
      });
      // console.log(row); // 在控制台中输出row对象的内容
      // this.roleId = row;
      // this.menuDialogVisible = true;
      // this.request.post("/role/selectMenuByRole/" + this.roleId).then(res => {
      //   console.log("选择菜单id");
      ...
      //   this.request.post("menu/findAl").then(res => {
      //     console.log("显示已经绑定的菜单id");
      //     this.menuData = res.data;
      //     console.log(this.menuData);
      //     // 根据已选中的菜单项来设置勾选状态
      //     this.$nextTick(() => {
      //       this.menuData.forEach(menu => {
      //         if (this.checks.includes(menu.id)) {
      //           this.$refs.tree.setChecked(menu.id, true);
      //         }
      //       });
      //     });
      //   });
      // });
      // this.request.post("/role/selectMenuByRole/" + this.roleId).then(res => {
      //   this.checks = res.data.map(row => row.menu_id);
      //   console.log(this.checks);
      //   this.request.post("menu/findAl").then(res => {
      ...
      //         this.$refs.tree.setChecked(menu.id, true);
      //       } else {
      //         this.$refs.tree.setChecked(menu.id, false);
      //       }
      //     });
      //   });
      // });
      // this.request.post("/role/selectMenuByRole/" + this.roleId).then(res => {
      //   const checkedMenus = res.data.map(row => row.menu_id);
      ...
      //     const checkedKeys = [];
      //     allMenus.forEach(menu => {
      //       if (checkedMenus.includes(menu.id)) {
    ...
    saveRoleMenu() {
      ...
      // alert(menuIds);
      this.request
        .post("/role/saveRoleMenu/" + this.roleId, menuIds)
        .then(res => {
          if (res.code === 200) {
