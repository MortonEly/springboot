...
<!-- 主体部分框架代码 -->
 ...
 <!-- :header-cell-style设置表头样式 -->
			...
			<el-table-column prop="nickname" label="姓名" width="150px">
			</el-table-column>
<el-table-column prop="role" label="角色" width="105px">
  <template slot-scope="scope">
    <span>
      <el-tag v-for="role in roleList" :key="role.id" v-if="role.id === scope.row.roleId" :width="100">
        {{ role.name }}
      </el-tag>
    </span>
  </template>
...
<el-select clearable v-model="form.roleId" placeholder="请选择角色" style="width:80%">
    <el-option v-for="role in roleList" :key="role.name" :label="role.name" :value="role.id"></el-option>
</el-select>
				</el-form-item>
      ...
      this.request
        ...
        .then(res => {
          if (res.code === 200) {
            ...
            this.$message.error(res.data.msg); // 使用后端返回的消息提示保存失败原因
          }
          this.dialogFormVisible = false;
          this.load();
        })
