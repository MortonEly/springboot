...
@ApiModel(value="log字典对象", description="log字典方法")
@RestController
@RequestMapping("/log")
public class SysLogController {
    ...
    public Result save(@RequestBody SysLog sysLog) {
        //字典名重复问题
        boolean  b= sysLogService.saveOrUpdate(sysLog);
        if (b){
            return Result.success(200,"保存log信息成功",sysLog); // 使用无参的 success() 方法
        }else
        {
            return  Result.error();
        }
    ...
    public Result findAll(@RequestParam(name = "type",defaultValue = "")String type){
        ...
        return Result.success(200,"查询dict中icon成功",sysLogService.list(queryWrapper));
    }
    ...
    public Result deleteBatch(@RequestBody List<Integer> idList) {
        ...
        boolean delete = sysLogService.removeByIds(idList);
        if(delete)
        {
    ...
    public Result findPage(@RequestParam Integer pageNum,
        ...
        IPage<SysLog> rolePage = sysLogService.page(page, queryWrapper);
