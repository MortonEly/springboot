...
public class UserServiceImpl extends ServiceImpl<UserMapper, User> implements IUserService {
     ...
     * 检查用户名是否已存在
     * @param username 用户名
     * @param userId 用户ID（如果是更新操作）
     * @return 如果已存在返回true，否则返回false
     */
    private boolean isUsernameExists(String username, Long userId) {
    ...
    public Result login(UserControllerLogin userControllerLogin) {
        ...
        if (userList !=null){
        // 登录成功
            String token = TokenUtil.generateToken(userList.getId().toString(), password, 12*1200); // 设置token过期时间为1天（86400000毫秒）
    ...
    * */
    public long countUsers() {
        return count(); // 直接调用MyBatis-Plus提供的count方法来统计用户数量
    }
