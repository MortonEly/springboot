...
@Service
@Slf4j
public class Swagger3ApiParser {
    ...
    public Map<String, Attribute> parseOpenApiDefinitions(Map<String, Object> map) {
        //解析model
        Map<String, Map<String, Object>> components = (Map<String, Map<String, Object>>) map.get("components");
        Map<String, Map<String, Object>> schemas = null;
        if (components != null) {
            for (Map.Entry<String, Object> entry : map.entrySet()) {
                Map<String, Object> componentMap = (Map<String, Object>) entry.getValue();
                schemas = (Map<String, Map<String, Object>>) componentMap.get("schemas");
                if (schemas != null) {
                    break;
                }
            }
        }
        Map<String, Attribute> resMap = new HashMap<>(256);
        if (schemas != null) {
            Iterator<String> modelNameIt = schemas.keySet().iterator();
            while (modelNameIt.hasNext()) {
                String modeName = modelNameIt.next();
                buildAttribute(schemas, resMap, modeName);
            }
        }
        return resMap;
    }
     ...
     * 对$ref类型设置具体属性
     */
    public Attribute buildAttribute(Map<String, Map<String, Object>> swaggerMap, Map<String, Attribute> resMap, String modeName) {
        Attribute modeAttr;
        ...
        Map<String, Object> attrInfoMap = swaggerMap.get(modeName);
        if (attrInfoMap == null) {
            return null;
        }
        List<Attribute> attrList = new ArrayList<>();
        Attribute child = new Attribute();
        child.setName(modeName);
    ...
    public List<Table> getResult(Map<String, Object> map) throws IOException {
        //解析model
    ...
    private List<Table> getResultGroupByPath(Map<String, Object> map) throws IOException {
        //解析model
        Map<String, Map<String, Object>> definitions = (Map<String, Map<String, Object>>) map.get("definitions");
        Map<String, Attribute> definitionMap = parseDefinitions(definitions);
...
//        List<String> defaultProduces = (List<String>) map.get(PRODUCES);
        List<String> defaultProduces = null;
        return buildDefinitionsAndProcessByGroupPath(paths, definitionMap, defaultConsumes, defaultProduces);
    }

    public List<Table> buildDefinitionsAndProcessByGroupPath(Map<String, Map<String, Object>> paths, Map<String, Attribute> definitionMap, List<String> defaultConsumes, List<String> defaultProduces) throws IOException {
    ...
    private Table buildGroupTable(String url, Map<String, Attribute> definitionMap, Map.Entry<String, Object> request, List<Map<String, Object>> commonParameters, List<String> defaultConsumes, List<String> defaultProduces) throws IOException {
    ...
    public List<Table> buildDefinitionsAndProcessByPath(Map<String, Map<String, Object>> paths, Map<String, Attribute> definitionMap)  {
        List<Table> results = new ArrayList<>();
        if (paths != null) {
            Iterator<Map.Entry<String, Map<String, Object>>> it = paths.entrySet().iterator();
            while (it.hasNext()) {
    ...
    private Table buildTable(String url, Map<String, Attribute> definitionMap, Map.Entry<String, Object> request)  {
        ...
        // 7.请求参数格式，类似于 multipart/form-data
        String requestForm = "";
        Map<String, Object> consumes = (Map<String, Object>) content.get("requestBody");
        if (consumes != null) {
            requestForm = StringUtils.join(consumes.get("content"), ",");
        }
        ...
        String responseForm = "";
        if (obj != null) {
            responseForm = StringUtils.join(obj.get("content"), ",");
        }
        table.setResponseForm(responseForm);
        //示例
        table.setRequestParam(processRequestParam(table.getRequestList()));
