...
public class Swagger3Controller {
    private String fileName = "Swagger3Word";
    @Autowired
     ...
     * @param response
     */
    @ApiOperation(value = "将 open api3.0  json url地址转换成 word文档并下载")
    @ApiResponses(value = {@ApiResponse(code = 200, message = "请求成功。")})
    @GetMapping(value = "/urlToWord")
    public void wordOpenAPi(Model model, @ApiParam(value = "资源地址",example = "http://127.0.0.1:8893/v3/api-docs") @RequestParam String url, HttpServletResponse response) {
        Map<String, Object> result = documentService.getFromSwagger3Url(url);
    ...
    public void getWord(Model model, @ApiParam("swagger json file") @Valid @RequestPart("jsonFile") MultipartFile jsonFile, HttpServletResponse response) {
        Map<String, Object> result = documentService.getTableFromSwagger3JsonFile(jsonFile);
    ...
    public void getWord(Model model, @ApiParam("swagger json string") @Valid @RequestBody String jsonStr, HttpServletResponse response) {
        Map<String, Object> result = documentService.parseSwagger3JsonForTableFrom(jsonStr);
        model.addAttribute("url", "http://");
        model.addAllAttributes(result);
        String content = documentService.processForContent(model.asMap());
        writeContentToResponse(content, response);
